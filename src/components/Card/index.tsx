...
export const Card = ({
  ...
  const sizes =
    size === "sm"
      ? {
          card: "h-[150px] w-[105px] outline-[5px] py-2 sm:h-[260px] sm:w-[180px] sm:outline-8 sm:py-3 lg:h-[300px] lg:w-[210px] lg:outline-10 lg:py-4 lg:pt-[14px] lg:pb-[23px]",
          name: "w-[85px] text-[9px] sm:w-[130px] sm:text-xs lg:w-[150px] lg:text-[15px]",
          image: "h-[75px] w-[75px] sm:h-[130px] sm:w-[130px] lg:h-[150px] lg:w-[150px]",
          stats: "w-[63px] gap-1 sm:w-[105px] sm:gap-2 lg:w-[120px] lg:gap-[5px]",
          statRow: "h-3 text-[6px] sm:h-4 sm:text-[8px] lg:h-5 lg:text-[10px]",
          statBox: "w-[16px] border sm:w-[26px] sm:border-2 lg:w-[31px]",
          statBar:
            "bottom-0 left-1 w-[calc(100%-5px)] sm:left-2 sm:w-[calc(100%-9px)] lg:left-2.5 lg:w-[calc(100%-11px)]",
        }
      : {
          card: "h-[300px] w-[210px] outline-10 pt-[14px] pb-[23px]",
          name: "w-[150px] text-[15px]",
          image: "h-[150px] w-[150px]",
          stats: "w-[120px] gap-[5px]",
          statRow: "h-5 text-[10px]",
          statBox: "w-[31px] border-2",
          statBar: "bottom-0 left-2.5 w-[calc(100%-11px)]",
        };

  return (
    <div
      onClick={onClick}
      className={`flex ${sizes.card} shrink-0 flex-col items-center justify-between rounded-[10px] bg-space outline-white ${className} ${
        onClick ? "cursor-pointer" : ""
      }`}
    >
      <div className="flex flex-col items-center gap-2.5">
        <div className={sizes.name}>
          <p className="truncate text-left font-luckiest leading-none">
            {displayName}
          </p>
          <div className="h-[2px] w-full rounded-full border border-white bg-neon" />
        </div>
